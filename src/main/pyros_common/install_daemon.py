...
class InstallDaemon:
    def __init__(self, home_dir, user, group):
    ...
    def install(self):
        if not os.path.exists(self.home_dir):
            ...
            if not ensure_dir(self.home_dir, self.user, self.group, "code", verbose=True):
            ...
            if not os.path.exists("/etc/systemd/system/"):
                ...
                with Resource("linux-service/pyros.service") as in_file:
                    try:
                        ...
                        print("Running: systemctl enable pyros.py.service")
                        os.system("systemctl enable pyros.py.service")
